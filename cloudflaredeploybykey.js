...
// Load the Cloudflare API key from the environment variable
const cloudflareApiKey = process.env.CLOUDFLARE_API_KEY;
console.log('Cloudflare API key:', cloudflareApiKey);

if (!cloudflareApiKey) {
  console.error('Cloudflare API key is not set in environment variables.');
  process.exit(1);
}
...
fs.readFile('cloudflare/pages.json', 'utf8', (err, data) => {
  ...
  function deployPage(index) {
    ...
    const command = `CLOUDFLARE_API_KEY=${cloudflareApiKey} npx wrangler pages deploy _site/${page.host} --branch main --project-name ${page.project}`;
